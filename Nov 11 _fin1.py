...
def udp_receiver(port=5005, data_signal=None):
    ...
    while True:
        ...
        if len(measurement_group) >= 5:  # Example threshold for group size
            if data_signal:
                data_signal.new_data.emit(measurement_group)
            measurement_group = []
        ...
        for track_id, track in enumerate(self.tracks):
            if correlation_check(track, measurement, self.doppler_threshold, self.range_threshold):
                ...
                if current_state == 'Poss1':
                    # Correct the number of arguments passed
                    initialize_filter_state(self.kalman_filter, measurement[0], measurement[1], measurement[2], 0, 0, 0, measurement[3])
                elif current_state == 'Tentative1':
                    last_measurement = track['measurements'][-1][0]
                    dt = measurement[3] - last_measurement[3]
                    vx = (measurement[0] - last_measurement[0]) / dt
                    vy = (measurement[1] - last_measurement[1]) / dt
                    vz = (measurement[2] - last_measurement[2]) / dt
                    initialize_filter_state(self.kalman_filter, measurement[0], measurement[1], measurement[2], vx, vy, vz, measurement[3])
                ...
                log_data = {
                    'Time': measurement[3],
                    'Measurement X': measurement[0],
                    'Measurement Y': measurement[1],
                    'Measurement Z': measurement[2],
